...
class TestSpatialJoin:
    ...
    def test_empty_join(self):
        # Checks that joining with an empty gdf returns empty
        ...
        empty = geopandas.GeoDataFrame({"col1": [], "geometry": []})
        res = sjoin_nearest(empty, polygons, how="left")
        assert res.index_right.isnull().all()
        res = sjoin_nearest(empty, polygons, how="right")
        assert res.index_left.isnull().all()
        res = sjoin_nearest(empty, polygons, how="inner")
        assert res.empty
