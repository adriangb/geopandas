...
class TestMaxSearchNeighbors:
    ...
    @pytest.mark.skipif((compat.USE_PYGEOS), reason="Pygeos does not have this bug")
    ...
    def test_neighbors_small(self, how, n):
        ...
        """
        sjn_None = self.sjn_None[how]
        if not compat.RTREE_GE_094 and not compat.USE_PYGEOS:
    ...
    # n chosen empirically for this dataset
    @pytest.mark.skipif(
        not (compat.RTREE_GE_094 or compat.USE_PYGEOS), reason="Rtree <= 0.9.4"
    )
    ...
    # n chosen empirically for this dataset
    @pytest.mark.skipif(
        (compat.RTREE_GE_094 or compat.USE_PYGEOS), reason="Rtree > 0.9.4"
    )
    @pytest.mark.parametrize("n", [2, 10, 50])
