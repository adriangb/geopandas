...
def sjoin_nearest(
    ...
    def query_index(geo_in_l):
        """
        Queries the spatial index to filter results.
        """
        # restric by radius
        if search_radius is not None:
            if geo_in_l.geom_type == "Point" and search_radius == 0:
                # applying .buffer(0) to a point returns an empty geometry
                geo_search = geo_in_l.coords[0]
            else:  # polygon or buffer > 0
                geo_search = geo_in_l.buffer(search_radius).bounds
            in_radius = list(tree_idx.intersection(geo_search))
        else:
    ...
    r_idx = []
    distances = []
    for ind_in_left, geo_in_l in enumerate(left_df.geometry):
        if not geo_in_l or geo_in_l.is_empty:
            # see https://github.com/Toblerity/Shapely/issues/799
            continue
        possible_nearest = query_index(geo_in_l)
        min_dist = np.inf  # initialize
        min_ind = []  # initialize
        for ind_in_right in possible_nearest:
            geo_in_r = right_df.geometry.iloc[ind_in_right]
