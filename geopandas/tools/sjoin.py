...
Currently, sjoin is implemented on 2 backends: rtree and pygeos.
"""
...
# pygeos defines within and contains as the opposite of what
# geopandas sjoin has used historically, so we flip them
pygeos_op_flip = {
    "within": "contains",
    "contains": "within",
}
...
def sjoin(
    ...
    if compat.USE_PYGEOS:
        return _sjoin_pygeos(left_df, right_df, how, op, lsuffix, rsuffix)
    elif compat.HAS_RTREE:
        return _sjoin_rtree(left_df, right_df, how, op, lsuffix, rsuffix)
    else:
        raise ValueError("Need `rtree` or `pygeos` to use `sjoin`")


def _sjoin_rtree(
    left_df, right_df, how="inner", op="intersects", lsuffix="left", rsuffix="right"
):
    """Implementation using pygeos spatial index.
    """
    # --------------------------------------------------------------------------
    # Check inputs
    # --------------------------------------------------------------------------
    ...
    # --------------------------------------------------------------------------
    # Rename indexes to integers for spatial index compatibility
    # --------------------------------------------------------------------------
    (
        left_df,
        right_df,
    ...
    # --------------------------------------------------------------------------
    # Compute spatial join (handle `op` param)
    # --------------------------------------------------------------------------
    ...
    # --------------------------------------------------------------------------
    # Compile results (handle `how` param)
    # --------------------------------------------------------------------------
...
def _sjoin_pygeos(
    ...
    # pygeos defines within and contains as the opposite of what
    # geopandas sjoin has used historically, so we flip them
    op = pygeos_op_flip.get(op, op)

    if right_df._sindex_generated or (
        not left_df._sindex_generated and right_df.shape[0] > left_df.shape[0]
    ):
        tree_idx = right_df.sindex if len(left_df) > 0 else None
        query_geom = left_df.geometry.values.data
        l_idx, r_idx = tree_idx.query_bulk(query_geom, op)
    else:
        tree_idx = left_df.sindex if len(right_df) > 0 else None
        query_geom = right_df.geometry.values.data
        r_idx, l_idx = tree_idx.query_bulk(query_geom, op)

    if l_idx.size and r_idx.size:
        # indexes of overlapping boundaries
        res_vec = np.stack((l_idx, r_idx), axis=1)
...
def _sjoin_nearest_pyges(
    ...
    if tree_idx is not None:
        l_idx, r_idx, distances = tree_idx.nearest(left_df.geometry.values.data, True)

        if search_radius is not None:
            in_radius = distances <= search_radius
            l_idx = l_idx[in_radius]
            r_idx = r_idx[in_radius]
